...
COMPOSITOR_ENABLED="true"  # valeur par défaut si pas de conf
if [ -f "$CONF_FILE" ]; then
    # shellcheck source=/dev/null
    source "$CONF_FILE"
fi
...
if [ "$1" != "no-compositor" ] && [ "$COMPOSITOR_ENABLED" = "true" ]; then
    ...
    # S'assurer qu'il ne tourne pas (cas d'un retour depuis un autre DE)
    killall picom compton xcompmgr 2>/dev/null
fi

# Notification daemon (si pas déjà lancé)
if ! pgrep -x "notification-daemon" > /dev/null 2>&1 && \
   ! pgrep -x "dunst"               > /dev/null 2>&1 && \
   ! pgrep -x "notify-osd"          > /dev/null 2>&1; then
    if command -v dunst &> /dev/null; then
        start_process "dunst" "Dunst Notification Daemon"
    fi
fi
...
if command -v nitrogen &> /dev/null; then
    start_process_args "Nitrogen Wallpaper" nitrogen --restore
elif command -v feh &> /dev/null; then
    if [ -f ~/.fehbg ]; then
        cp ~/.fehbg ~/.fehbg-hybryde
        ~/.fehbg &
        echo "✓ Démarré: Feh Wallpaper"
    fi
fi
