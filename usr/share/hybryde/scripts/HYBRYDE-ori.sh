...
openbox()
{
    ...
    # L'autostart XDG ou ~/.config/openbox/autostart peut lancer un agent polkit.
    # Si un doublon apparaît pendant les 10 premières secondes, on tue le surplus
    # et on garde le premier agent enregistré auprès de polkitd.
    (
        for i in $(seq 1 10); do
            sleep 1
            pids=$(pgrep -f "xfce-polkit|polkit-mate-authentication|polkit-gnome-authentication" \
                   2>/dev/null | tr '\n' ' ')
            count=$(echo "$pids" | wc -w)
            if [ "$count" -gt 1 ]; then
                echo "[anti-polkit] Doublon détecté (${count} agents) — suppression des extras"
                first=true
                for pid in $pids; do
                    if $first; then
                        first=false
        ...
        echo "[anti-polkit] Surveillance terminée"
    ) &
