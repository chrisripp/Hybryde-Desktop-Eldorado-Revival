...
tuer_de() {
    ...
    case "$de" in
        ...
        mate*)
            for p in mate-session marco mate-panel mate-settings-daemon \
                     mate-notification-daemon mate-polkit; do
                killall "$p" 2>/dev/null
            done ;;
        ...
        cinnamon*)
            ...
            sleep 2
            for p in cinnamon-session-binary cinnamon-session cinnamon muffin nemo \
                     cinnamon-settings-daemon cinnamon-screensaver; do
                killall "$p" 2>/dev/null
            done ;;
        ...
        gnome*|ubuntu*)
            ...
            for p in gnome-flashback gnome-session-ctl gnome-session-service \
                     gnome-session-init-worker gnome-shell mutter gnome-panel metacity \
                     gsd-background gsd-color gsd-datetime gsd-housekeeping \
                     gsd-print-notifications gsd-rfkill gsd-screensaver-proxy \
                     gsd-sharing gsd-smartcard gsd-sound gsd-wacom; do
                killall "$p" 2>/dev/null
            done ;;
