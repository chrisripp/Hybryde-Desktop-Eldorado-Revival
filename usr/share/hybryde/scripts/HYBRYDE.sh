...
killprocess()
{
    ...
    # Attendre la mort effective des agents polkit avant de continuer
    # (évite "authentication agent already exists" lors du switch)
    local polkit_die=0
...
gnome_panel()
{
    ...
    if command -v gnome-panel &> /dev/null; then
        ...
        zenity --error --text="GNOME Panel n'est pas installé\nPaquets requis: gnome-panel"
        rm -f "$SWITCHING_FLAG"
        return 1
    fi
...
gnome3()
{
    ...
    if ! command -v gnome-session &> /dev/null; then
        zenity --error --text="gnome-session n'est pas installé\nsudo apt install gnome-session"
        return 1
    fi
    if ! command -v gnome-shell &> /dev/null; then
        zenity --error --text="gnome-shell n'est pas installé\nsudo apt install gnome-shell"
        return 1
    fi
...
kde()
{
    ...
    if command -v startplasma-x11 &> /dev/null; then
        ...
        zenity --error --text="KDE Plasma n'est pas installé"
        rm -f "$SWITCHING_FLAG"
        return 1
    fi
...
enlightenment()
{
    ...
    if command -v enlightenment_start &> /dev/null; then
        ...
        zenity --error --text="Enlightenment n'est pas installé"
        rm -f "$SWITCHING_FLAG"
        return 1
    fi
...
openbox()
{
    ...
    echo "Pour revenir à Hybryde : lancez 'retour-hybryde' ou tuez openbox"
    ...
    # Masquer les entrées polkit de /etc/xdg/autostart AVANT le lancement
    # d'openbox-session, pour éviter la collision avec notre propre agent.
    # On crée un répertoire XDG temporaire qui remplace ces entrées par Hidden=true.
    _TMPXDG=$(mktemp -d /tmp/hybryde-xdg-XXXXX)
    mkdir -p "$_TMPXDG/autostart"
    for _f in /etc/xdg/autostart/*polkit*.desktop /etc/xdg/autostart/*policykit*.desktop; do
        [ -f "$_f" ] || continue
        _bname=$(basename "$_f")
        cp "$_f" "$_TMPXDG/autostart/$_bname"
        printf '\nHidden=true\n' >> "$_TMPXDG/autostart/$_bname"
        echo "  [polkit-block] Masqué pour Openbox : $_bname"
    done
    export XDG_CONFIG_DIRS="$_TMPXDG:${XDG_CONFIG_DIRS:-/etc/xdg}"

    # Lancer notre propre agent polkit EN PREMIER, avant openbox-session.
    # Ainsi, si ~/.config/openbox/autostart en lance un autre, c'est lui qui
    # échoue (déjà enregistré) et non le nôtre.
    if [ -x /usr/libexec/xfce-polkit ]; then
        /usr/libexec/xfce-polkit &
        echo "✓ xfce-polkit démarré (pré-Openbox)"
    elif [ -x /usr/libexec/polkit-mate-authentication-agent-1 ]; then
        /usr/libexec/polkit-mate-authentication-agent-1 &
        echo "✓ polkit-mate démarré (pré-Openbox)"
    fi
    sleep 0.5
    ...
    # Nettoyage du répertoire XDG temporaire
    rm -rf "$_TMPXDG"
...
cinnamon()
{
    ...
    if ! command -v cinnamon-session &> /dev/null; then
        zenity --error --text="Cinnamon n'est pas installé\nsudo apt install cinnamon"
        return 1
    fi
...
show_menu()
{
    CHOICE=$(zenity --list \
        ...
        --text="Choisissez un environnement de bureau :\n\nPour revenir à Hybryde : utilisez 'retour-hybryde'\n\nNote: KDE et GNOME peuvent prendre 30-60 secondes à démarrer" \
        --column="ID" --column="Environnement" --column="Description" \
        "gnome_panel" "GNOME Classique" "Interface traditionnelle GNOME (lent)" \
        "gnome3" "GNOME Shell" "Interface moderne GNOME (lent)" \
        "kde" "KDE Plasma" "Bureau KDE complet (lent)" \
        "xfce" "XFCE" "Bureau léger et rapide" \
        "lxde" "LXDE" "Bureau très léger" \
        "lxqt" "LXQt" "LXDE modernisé" \
        "mate" "MATE" "Fork de GNOME 2" \
        "cinnamon" "Cinnamon" "Bureau Linux Mint" \
        "openbox" "Openbox" "Gestionnaire de fenêtres minimal" \
        "enlightenment" "Enlightenment" "Bureau élégant" \
